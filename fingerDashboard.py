...
dui = pd.read_sql_query('SELECT count(*) FROM interviews_interviews where day(created_at)=day(Now()) and month(created_at)=month(Now()) and year(created_at)=year(Now())', conn)
duiVal = int(dui['count(*)'][0])

# 오늘 등록된 인터뷰 시간 (Daily Uploaded Time)
dut = pd.read_sql_query('SELECT sum(duration) FROM interviews_interviews where day(created_at)=day(Now()) and month(created_at)=month(Now()) and year(created_at)=year(Now())', conn)
